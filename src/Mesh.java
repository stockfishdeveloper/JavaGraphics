...
class Mesh
{
	...
	public Mesh(String filename, Color c, double scalefactor) throws IOException
	{
		...
		while((line = buffer.readLine()) != null)
		{
			...
			{ 
				vertices.add(new Point(Double.parseDouble(values[1]) * scalefactor, Double.parseDouble(values[2]) * scalefactor, Double.parseDouble(values[3]) * scalefactor));
			}
			else if(values[0].equals("f"))
			{
				triangles.add(new Triangle(vertices.get(Integer.parseInt(values[1]) - 1), 
				vertices.get(Integer.parseInt(values[2]) - 1), vertices.get(Integer.parseInt(values[3]) - 1), color));
			}
		}
		buffer.close();
	}
