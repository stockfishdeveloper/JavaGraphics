...
class SkyBox {
	...
	void Render(Graphics2D g, Point cameradirection) {
		...
		if (cameradirection.GetExZ() > 0) {
			if (cameradirection.GetExX() > 0) {// DONE
				leftmost = front;
				rightmost = right;
				double tan = Math.toDegrees(Math.atan(cameradirection.GetExX() / cameradirection.GetExZ()));
				int coorx = (int) (tan * -14.222222);
				double ycoor = (cameradirection.GetExY() > 0 ? 1 : -1) * 11.111111
						* Util.Get_Angle_Between_Vectors(cameradirection, new Point(0, 0, 0),
								new Point(cameradirection.GetExX(), 0, cameradirection.GetExZ()));
				
				g.drawImage(leftmost, coorx, (int) (ycoor), 1280, 1000, null);
				g.drawImage(rightmost, coorx + 1280, (int) (ycoor), 1280, 1000, null);
				g.drawImage(toporbot, coorx, toporbot == up ? (int) (ycoor - 1000) : (int) (ycoor + 1000), 1280, 1000,
						null);
				// g.drawImage(toporbot, coorx + 1280, toporbot == up ? (int)(ycoor - 1000) :
				// (int)(ycoor + 1000), 1280, 1000, null);
			} else {
				leftmost = left;
				rightmost = front;
				double tan = Math.toDegrees(Math.atan(cameradirection.GetExX() / cameradirection.GetExZ()));
				...
				g.drawImage(toporbot, coorx, toporbot == up ? (int) (ycoor - 1000) : (int) (ycoor + 1000), 1280, 1000,
						null);
				// g.drawImage(toporbot, coorx, toporbot == up ? (int)(ycoor - 1000) :
				// (int)(ycoor + 1000), 1280, 1000, null);
			}
		} else {
			if (cameradirection.GetExX() > 0) {
				leftmost = back;
				rightmost = right;
				Point turned = new Point(-cameradirection.GetExX(), cameradirection.GetExY(),
						-cameradirection.GetExZ());
				...
				g.drawImage(leftmost, coorx + 1280, (int) (ycoor), 1280, 1000, null);
				g.drawImage(rightmost, coorx, (int) (ycoor), 1280, 1000, null);
				g.drawImage(toporbot, coorx + 1280, toporbot == up ? (int) (ycoor - 1000) : (int) (ycoor + 1000), 1280,
						1000, null);
				// g.drawImage(toporbot, coorx, toporbot == up ? (int)(ycoor - 1000) :
				// (int)(ycoor + 1000), 1280, 1000, null);
			} else {
