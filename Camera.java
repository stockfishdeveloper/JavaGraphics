...
class Camera {
	...
	public void RotateCounterClockwiseAboutZAxis(Point p, float degrees) {
		if (Util.Get_Angle_Between_Vectors(direction, new Point(0, 0, 0),
				new Point(direction.GetExX(), 0, direction.GetExZ())) + degrees <= 90) {
	...
	public void RotateCounterClockwiseAboutUpAxis(float degrees) {
		 ...
		 * RotateCounterClockwiseAboutYAxis(eye, (float)(90d -
		 * Util.RadiansToDegrees(tan))); ry = new Rotation(false, Axis.Y, (float)(90d -
		 * Util.RadiansToDegrees(tan))); } else if(direction.GetExX() < 0) {
		 * RotateClockwiseAboutYAxis(eye, (float)(90d + Util.RadiansToDegrees(tan))); ry
		 * = new Rotation(true, Axis.Y, (float)(90d + Util.RadiansToDegrees(tan))); } }
		 * else if(direction.GetExZ() < 0) { if(direction.GetExX() < 0) {
		 * RotateClockwiseAboutYAxis(eye, (float)(Util.RadiansToDegrees(tan) + 90d)); ry
		 * = new Rotation(true, Axis.Y, (float)(Util.RadiansToDegrees(tan) + 90d)); }
		 * else if(direction.GetExX() > 0) { RotateCounterClockwiseAboutYAxis(eye,
		 * (float) (-Util.RadiansToDegrees(tan) + 90d)); ry = new Rotation(false,
		 * Axis.Y, (float)(-Util.RadiansToDegrees(tan) + 90d)); } } tan =
		 * Math.atan(direction.GetExY() / direction.GetExZ()); if(direction.GetExY() >
		 * 0) { if(direction.GetExZ() > 0) { RotateClockwiseAboutXAxis(eye,
		 * (float)(Util.RadiansToDegrees(tan))); rx = new Rotation(true, Axis.X,
		 * (float)(Util.RadiansToDegrees(tan))); } else if(direction.GetExZ() < 0) {
		 ...
		 * else if(direction.GetExY() < 0) { if(direction.GetExZ() < 0) {
		 * RotateClockwiseAboutXAxis(eye, (float)(Util.RadiansToDegrees(tan))); rx = new
		 * Rotation(true, Axis.X, (float)(Util.RadiansToDegrees(tan))); } else
		 * if(direction.GetExZ() > 0) { RotateCounterClockwiseAboutXAxis(eye,
		 * (float)(-Util.RadiansToDegrees(tan))); rx = new Rotation(false, Axis.X,
	...
	public void RotateCounterClockwiseAboutRightAxis(float degrees) {
		double angle = Util.Get_Angle_Between_Vectors(direction, new Point(0, 0, 0),
				new Point(direction.GetExX(), 0, direction.GetExZ()));
		 ...
		 * = new Rotation(true, Axis.Y, (float)(Util.RadiansToDegrees(tan) + 90d)); }
		 * else if(direction.GetExX() > 0) { RotateCounterClockwiseAboutYAxis(eye,
		 * (float)(-Util.RadiansToDegrees(tan) + 90d)); ry = new Rotation(false, Axis.Y,
		 * (float)(-Util.RadiansToDegrees(tan) + 90d)); } } tan =
		 * Math.atan(direction.GetExY() / direction.GetExZ()); if(direction.GetExY() >
		 * 0) { if(direction.GetExZ() > 0) { RotateClockwiseAboutXAxis(eye,
		 * (float)(Util.RadiansToDegrees(tan))); rx = new Rotation(true, Axis.X,
		 * (float)(Util.RadiansToDegrees(tan))); } else if(direction.GetExZ() < 0) {
	...
	public void RotateClockwiseAboutRightAxis(float degrees) {
		 ...
		 * (float)(Util.RadiansToDegrees(tan))); } else if(direction.GetExZ() < 0) {
		 * RotateCounterClockwiseAboutXAxis(eye, (float)(-Util.RadiansToDegrees(tan) +
		 * 180)); rx = new Rotation(false, Axis.X, (float)(-Util.RadiansToDegrees(tan) +
		 * 180)); } } else if(direction.GetExY() < 0) { if(direction.GetExZ() < 0) {
		 * RotateClockwiseAboutXAxis(eye, (float)(Util.RadiansToDegrees(tan))); rx = new
		 * Rotation(true, Axis.X, (float)(Util.RadiansToDegrees(tan))); } else
		 * if(direction.GetExZ() > 0) { RotateCounterClockwiseAboutXAxis(eye,
		 * (float)(-Util.RadiansToDegrees(tan))); rx = new Rotation(false, Axis.X,
	...
	public Triangle LookAt(Triangle triangle) {
		/*
		 * for(Point p : triangle.points) { if(frustum.Contains(p) == true) return null;
		 * }
		 */
		/*
		 * if(!(frustum.Contains(triangle.points[0]) ||
		 * frustum.Contains(triangle.points[1]) ||
		 * frustum.Contains(triangle.points[2]))) { return null; }
		 */
		...
		for (Point p : object.points) {
			...
			if ((p.GetExX() < 0) && (p.GetExX() > p.GetExZ()))
				return null;
			/*
			 * if((p.GetExZ() * 0.78) + 25 < Location.GetExY()) return null; if((p.GetExZ()
			 * * -0.78) + 25 < Location.GetExY()) return null;
			 */
		}
